...
CREATE TABLE comment (id BIGINT AUTO_INCREMENT, text text NOT NULL, update_at DATE NOT NULL, PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE person (id BIGINT AUTO_INCREMENT, first_name VARCHAR(255) NOT NULL, last_name VARCHAR(255) NOT NULL, email_address VARCHAR(255) NOT NULL UNIQUE, PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE static_page (id BIGINT AUTO_INCREMENT, position BIGINT NOT NULL, publication_date DATETIME, category_id BIGINT, INDEX category_id_idx (category_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
CREATE TABLE sf_guard_user (id BIGINT AUTO_INCREMENT, username VARCHAR(128) NOT NULL UNIQUE, algorithm VARCHAR(128) DEFAULT 'sha1' NOT NULL, salt VARCHAR(128), password VARCHAR(128), is_active TINYINT(1) DEFAULT '1', is_super_admin TINYINT(1) DEFAULT '0', last_login DATETIME, user_id BIGINT, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, INDEX is_active_idx_idx (is_active), INDEX user_id_idx (user_id), PRIMARY KEY(id)) ENGINE = INNODB;
...
ALTER TABLE sf_guard_user ADD CONSTRAINT sf_guard_user_user_id_person_id FOREIGN KEY (user_id) REFERENCES person(id);
